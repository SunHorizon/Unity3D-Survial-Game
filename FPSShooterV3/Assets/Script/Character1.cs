...
public class Character1 : MonoBehaviour
{
    ...
    {     
        ...
        if (Input.GetButtonDown("Crouch1") && !water && !Lava)
        {
            if (isCrouch == false)
            {
        ...
        if (CheckKills.playerkills2 <= 5)
        {
            if (Health <= 0)
            {
                Instantiate(this, PlayerTwoSpawn.position, PlayerTwoSpawn.rotation);
                CheckKills.playerkills1++;
                Destroy(gameObject);
            }
    ...
    void Movement()
    {
        ...
        // moving using WASD
        MoveV = Input.GetAxis("Vertical1") * speed;
        MoveH = Input.GetAxis("Horizontal1") * speed;

        // moving using mouse
        rotX = Input.GetAxis("Controller X") * roatationSpeed;
        rotY -= Input.GetAxis("Controller Y") * roatationSpeed;
