...
public class Gun1 : MonoBehaviour {
	...
	void Update () {
        ...
        if (Time.deltaTime == 0)
        {
            if (Input.GetButtonDown("Fire1C") && FireCheck == true)
            {
        ...
        {
            if (Input.GetButtonDown("Fire1C") && FireCheck == true && !isReloading && clipAmmo > 0)
            {
        ...
        if (Input.GetButtonDown("Reload") && !isReloading && clipAmmo < 30 && TotalAmmo > 0)
        {
    ...
    void Fire()
    {
        ...
        if(Physics.Raycast(fpsCam.transform.position, fpsCam.transform.forward, out hit, range))
        {
            ...
            Character Player1 = hit.transform.GetComponent<Character>();
